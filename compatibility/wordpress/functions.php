...
if (!function_exists('wp_kses_post')) { function wp_kses_post(mixed $value): string { return strip_tags((string) $value, '<p><br><a><strong><em><ul><ol><li><blockquote><code><pre><h1><h2><h3><h4><h5><h6>'); } }
...
if (!function_exists('wp_enqueue_script')) { function wp_enqueue_script(string $handle, string $src = '', array $deps = [], string|bool|null $ver = false, bool $in_footer = false): void { cajeer_wp_runtime()->addAction('wp_enqueue_scripts', static fn () => null); } }
if (!function_exists('wp_enqueue_style')) { function wp_enqueue_style(string $handle, string $src = '', array $deps = [], string|bool|null $ver = false, string $media = 'all'): void { cajeer_wp_runtime()->addAction('wp_enqueue_styles', static fn () => null); } }
