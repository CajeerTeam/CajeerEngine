...
{
    private DleTemplateParser $parser;

    public function __construct(?string $themeRoot = null)
    {
        $this->parser = new DleTemplateParser($themeRoot);
    }

    public function renderTemplateFile(string $file, array $data): string
    {
        if (!is_file($file)) throw new \RuntimeException("DLE-шаблон не найден: {$file}");
    ...
    public function scanTemplate(string $file): array
    {
        ...
        return ['tags' => $this->parser->detectTags($template), 'report' => $this->parser->compatibilityReport($template)];
    }
}
