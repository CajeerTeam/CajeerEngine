...
#[AsCommand(name: 'stable:support-bundle', description: 'Создать support bundle без секретов.')]
final class StableSupportBundleCommand extends BaseCommand
{
    protected function configure(): void
    {
        ...
        $this->addOption('no-logs', null, InputOption::VALUE_NONE, 'Не включать runtime/audit logs.');
    }

    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        $service = new StableReleaseService($this->rootPath, new ConfigRepository($this->rootPath));
        $result = $service->supportBundle(['include_logs' => !(bool) $input->getOption('no-logs')]);
        ...
        if ((bool) $input->getOption('json')) {
            ...
            $output->writeln('Stable support bundle: ' . ($ok ? 'OK' : 'FAILED'));
