...
#[AsCommand(name: 'extension:enable', description: 'Включить установленное расширение.')] 
final class ExtensionEnableCommand extends BaseCommand
{
    ...
    {
        try {
            $item = (new ExtensionRegistry($this->rootPath, new ConfigRepository($this->rootPath)))->enable((string) $input->getArgument('name'));
            if ((bool) $input->getOption('json')) {
                $output->writeln(json_encode(['data' => $item], JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES));
            } else {
                $output->writeln('[OK] Включено расширение: ' . (string) ($item['name'] ?? ''));
            }
            return self::SUCCESS;
        } catch (\Throwable $e) {
