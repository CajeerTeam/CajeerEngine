...
#[AsCommand(name: 'extension:assets', description: 'Опубликовать assets расширений в public/extensions.')]
final class ExtensionAssetsCommand extends BaseCommand
{
    protected function configure(): void
    {
        $this->addArgument('name', InputArgument::OPTIONAL, 'Имя расширения vendor/name или slug');
        $this->addOption('dry-run', null, InputOption::VALUE_NONE, 'Показать план без копирования');
    ...
    {
        try {
            $result = $this->runtime()->publishAssets((string) ($input->getArgument('name') ?: '') ?: null, (bool) $input->getOption('dry-run'));
            if ((bool) $input->getOption('json')) {
                ...
                foreach ((array) ($result['data'] ?? []) as $item) {
                    $output->writeln(sprintf('[OK] %s: %d files -> %s', (string) ($item['extension'] ?? ''), (int) ($item['files'] ?? 0), (string) ($item['target'] ?? '')));
                }
            }
            return self::SUCCESS;
        } catch (\Throwable $e) {
