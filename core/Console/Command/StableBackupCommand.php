...
#[AsCommand(name: 'stable:backup', description: 'Создать backup runtime-данных CajeerEngine.')]
final class StableBackupCommand extends BaseCommand
{
    protected function configure(): void
    {
        ...
        $this->addOption('no-uploads', null, InputOption::VALUE_NONE, 'Не включать public/uploads.');
    }

    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        $service = new StableReleaseService($this->rootPath, new ConfigRepository($this->rootPath));
        $result = $service->backup(['include_uploads' => !(bool) $input->getOption('no-uploads')]);
        ...
        if ((bool) $input->getOption('json')) {
            ...
            $output->writeln('Stable backup: ' . ($ok ? 'OK' : 'FAILED'));
